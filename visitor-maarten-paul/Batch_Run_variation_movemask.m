...
for z=7:length(datasets)
    ...
    %     end
        ...
        for m=1:2
         ...
         imBWGT = ground_truth_random_move(imBW);
         %imSize = size(im);
         %iterations = floor(imSize(5)/100);
         %rest = mod(imSize(5),100);
         %imGT = zeros(imSize(1),imSize(2),imSize(3),imSize(4),imSize(5));
         %for l=1:iterations
         %    imGT(:,:,:,:,(1+(l-1)*100):(l*100)) = applyAffineTransformationMP(im(:,:,:,:,(1+(l-1)*100):(l*100)),random);
         %end
         %if(rest>0)
         %   imGT(:,:,:,:,(iterations*100+1):(iterations*100+rest)) = applyAffineTransformationMP(im(:,:,:,:,(iterations*100+1):(iterations*100+rest)),random);
         %end
            ...
            disp(['correlate tracks with mask at: ' datestr(now,'HH:MM:SS')]);
            trackData = CoorelateTrackWithMaskMPv2(trackData,imGT,imBWGT,pixelSize,cropFromEdge,startFrame);
            disp(['save tracks: ' datestr(now,'HH:MM:SS')]);
